...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "resume":
        await query.message.reply_text("Отправьте PDF файл резюме.")
    elif query.data == "vacancy":
        await query.message.reply_text("Отправьте ссылку или текст вакансии.")
    elif query.data == "report":
        user = query.from_user.id
        if user not in result_storage:
            await query.message.reply_text("Сначала сделайте анализ.")
            return
...
async def handle_pdf(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Резюме получено ✅\nТеперь отправьте вакансию.")
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    vacancy = parse_vacancy(update.message.text)
